...
func Router(audience, domain string) http.Handler {
	...
	router.Handle("/api/messages/protected", middleware.ValidateJWT(audience, domain)(
		http.HandlerFunc(middleware.ProtectedApiHandler)))
	router.Handle("/api/messages/admin", middleware.ValidateJWT(audience, domain)(
